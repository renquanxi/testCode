	...
	function TouchPDF(element, options) {
		...
		*/
		...
		function resetmypdf(){
			...
			$("#myPDF").css("left", 0).css('top', 0);
			var transformStyle='scale(1) rotate(' + cat.touchjs.rotateVal + 'deg)';
			$("#myPDF").css("transform", transformStyle).css("-webkit-transform", transformStyle);
		}
		function swiperFn(direction,needcz){
			if (direction == "right"){
				if(needcz){
					resetmypdf();
				}
				...
				if(pageNum==totalPages){
					mui.toast('您已经学习到最后一页！');
				}else {
					if (pageNum + 1 <= parseInt(end_page)) {
						if (needcz) {
							resetmypdf();
						}
						goto(pageNum + 1);
					} else {
						if (fileview_studystate == true) {
							var end_time = new Date();
							var datecha = end_time - start_time;
							var dates = datecha / 1000;
							if (dates > 5) {
								if (needcz) {
									resetmypdf();
								}
								...
								mui.toast('第一次学习，每个页面需要超过5s才能换页哦');
							}
						} else {
							if (needcz) {
								resetmypdf();
			...
			if (options.showToolbar) {
				...
				$element.find(".pdf-toolbar").html(
					'<div class="pdf-title">'+options.title+'</div>'
				 	...
				 	+ '<div class="pdf-button"><span class="pdf-page-count"></span></div>'
				 	+ '<div class="pdf-button"> </div>'
				 	+ (options.disableZoom? '':'<div class="pdf-button"> </div>'
				 		+ '<div class="pdf-button"> </div>')
				 	);
					...
					swipe:function(event, direction, distance, duration, fingerCount, fingerData) {
						...
						linksDisabled = true;
		      			...
		      			if(cat.touchjs.scaleVal<1){
							swiperFn(direction,false);
						}else {
							var keyslst = Object.keys(fingerData);
							var lastmovekey = keyslst[keyslst.length-1];
							var lastmovedata = fingerData[lastmovekey];
							var x = lastmovedata.end.x - lastmovedata.start.x;
							var y = lastmovedata.end.y - lastmovedata.start.y;
							var style = $("#myPDF").attr("style");
							var sfleft = style.split('left')[1].split(":")[1].split(';')[0];
							var sftop = style.split('top')[1].split(":")[1].split(';')[0];
							if (sfleft == undefined || sfleft == "") {
								sfleft = 0;
							}
							if (sftop == undefined || sftop == "") {
								sftop = 0;
							}
							var left = parseFloat(sfleft) + x;
							var top = parseFloat(sftop) + y;
							var scrollnum = cat.touchjs.scaleVal;
							var pdfwidth = $("#myPDF").width();
							var jdzleft = Math.abs(parseInt(parseInt(pdfwidth * (scrollnum - 1)) / 2));
							if (jdzleft <= Math.abs(left)) {
								swiperFn(direction,true);
							} else {
								$("#myPDF").css("left", left).css('top', top);
							}
						}
					},
					threshold:50,
		...
		*/
		...
		function redraw() {
			...
			$element.find(".pdf-outerdiv")
				// .css("transform", "scale("+scale+") translateY(-60%)")
